...
checkpointer = create_model_checkpoint('models', 'vae_seq2seq_test_stdof1')
...
vae.save_weights('models/vae_lstm_sample100.h5')
...
for n in range(0, 50000):
    if len(gen) < 500000:
        ...
        sent1 = train_data[num1].reshape(1,len(train_data[0]))
        sent2 = train_data[num2].reshape(1,len(train_data[0]))
        newintrp =  new_sents_generation(sent1, sent2, 20)
    gen.extend(newintrp)
    gen = list(set(gen))
    print(len(gen))
...
output = open('/nfs/home/6/tempker/aae/generated_text/VAE_generated/500000_generated_eq_20atatime.pkl','wb')
pk.dump(gen, output)   
