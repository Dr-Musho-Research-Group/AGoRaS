...
# print('There are ' +str(len(Eqns_clean ))+ ' cleaned equations')


# Eqns_clean = [x for x in Eqns if '~~' not in Eqns]


eqns = pd.Series(Eqns_clean)
eqns = eqns.replace(" ","", regex = True)
# eqns_old = pd.Series(Eqns_old)


Eqns_split = eqns.str.split('>', n=1, expand = True)
Eqns_split = Eqns_split.dropna()
# Eqns = Eqns.fillna(value = np.nan)

# Remove dubble equals
# CnEqns = Eqns[~Eqns[1].str.contains(">")]
...
# Name_List = FQTable.tolist()

# checking = [Smi2Form(x) for x in check]
...
# clean_dict = {val : key for key, val in clean_dict.items()} 
# species_list = list(clean_dict.values())
# species_list = [x[1] for x in species_list]



output = open('/nfs/home/6/tempker/aae/generated_text/created_species_lists/both/generated_species_list_for_all_species_asof_9820.pkl','wb')
